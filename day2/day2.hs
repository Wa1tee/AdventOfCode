--Day 2 challenge 1 for Advent of code 2017 by Antti Auranen
--adventofcode.com

import Data.List

--let list = [  "626 2424 2593 139 2136 163 1689 367 2235 125 2365 924 135 2583 1425 2502",
...
let list = ["5 9 2 8", "9 4 7 3", "3 8 6 5"]
...
-- Combine parsing and difference checking functions
asd :: [String] -> [Int]
asd x = map difference $ map parseString x

--Check for each line for the two values that are divisible
moro :: [String] -> [Int]
moro x = map divider $ map parseString x

--Parse the input line into integer list
parseString :: String -> [Int]
parseString x = map read $ words x ::[Int]

--Check for each line the difference between the maximum
--and the minimum value
difference :: [Int] -> Int
difference x = maximum x - minimum x

--Checks the list integers against n for any elements
--divisible by n
divCheck :: [Int] -> [Int]
divCheck integers n = [x | x <- integers, x `mod` n == 0]

--Find the output that has two elements from divCheck

divider :: [Int] -> Int
divider x = maximum x / minimum x

doubleFinder :: [Int] -> Int
doubleFinder [x,y]  = divider [x,y]

joku x = doubleFinder $ divCheck x